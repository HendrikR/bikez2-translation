...
void calculatebullets(void) // calculates bullets
{
    ...
    for (a = 0; a < MAX_BULLETS; a++) {
        ...
        if (a > 9) { // Bullets at the beginning are for mopeds to run over people
            ...
            if (bullet[a].distance > 20000) {
                bullet[a].remove = true;
                continue;
            }
            if ((bullet[a].place.x < 0) || (bullet[a].place.z < 0) || (bullet[a].place.x > mapsx * 8000) || (bullet[a].place.z > mapsz * 8000)) {
                bullet[a].remove = true;
                continue;
            }
        }
        ...
        if (ase[bullet[a].from_gun].damage <= 0) {
            if (bullet[a].place.y < 0) bullet[a].remove = true;
            continue;
        }
...
void setLights(float x, float y, float z)
{
    ...
    int q;
    for (q = 0; q < MAX_LIGHTS; q++)
        if (lights[q].on == false) break; // searching for free lightbulb (lights = lightbulb)
...
void move_moped(bikebase* moped)
{
    ...
    */
    ...
    for (q = 0; q < characters; q++) {
        ...
        {
            if (moped->target_character == -6667) {
    ...
    for (q = 0; q < characters; q++) {
        ...
        if (character[q].distance < 15000) {
            ...
            if (character[q].speed == 0) {
                ...
                BOOL eteen = false;
                if (character[q].leg)
                    if (character[q].lx < 0.10f) {
                        character[q].lx = character[q].lx + MOTION_SPEED * 0.02f * elapsed * gamespeed;
                        eteen           = true;
                    }
                if (!character[q].leg)
                    if (character[q].rx < 0.10f) {
                        character[q].rx = character[q].rx + MOTION_SPEED * 0.02f * elapsed * gamespeed;
                        eteen           = true;
                    }
...
void render_game_characters(void)
{
    ...
    for (a = 0; a < characters; a++) {
        ...
        if ((visiblecharacter[a] & D3DSTATUS_CLIPINTERSECTIONALL)) {
            character[a].visible = false;
            continue;
        }
...
void render_game_houses(float kerroin)
{
    ...
    for (int c = 0; c < 200; c++) {
        for (int a = 0; a < mapsz; a++) {
            for (int b = 0; b < mapsx; b++) {
                if (visiblemap[a * mapsx + b] & D3DSTATUS_CLIPINTERSECTIONALL) {
                    continue;
                }
...
void render_game_mopeds()
{
    ...
    if (gamephase2 != GP2_DEAD) // if player is not dead
        for (int d = 0; d < num_mopeds; d++) {
            if (moped[d].inactive) continue;
            moped[d].visible = true;
            if (visiblemap[d] & D3DSTATUS_CLIPINTERSECTIONALL) {
                moped[d].visible = false;
                continue;
            }
...
void render_game_bulletholes(float camq, float camw, float came)
{
    // if(smoke_count>0)
    {
        ...
        for (int q = 0; q < MAX_SMOKES; q++) {
            if (smokes[q].inactive) continue;
            if ((visible[q] & D3DSTATUS_CLIPINTERSECTIONALL)) {
                continue;
            }
...
{ // just renders some moped driving.
    float kerroin = 1.0;
    ase[2]._type  = 4;
    ase[3]._type  = 4;
    ase[4]._type  = 4;
...
void calc_coordinates(float* xa, float* za)
...
void calc_missions(bikebase* moped)
{ // calculate missions
    int q, f;
    bool kuollut = true;
    ...
    if (moped->mission._type == 0) {
        ...
        for (f = 0; f < moped->mission.kohteita; f++) {
            if (moped->mission.target_type == 0)
                if (!character[moped->mission.target[f]].dying) kuollut = false;
            if (moped->mission.target_type == 2)
                if (!moped[moped->mission.target[f]].inactive) kuollut = false;
        }
...
{ // calculate mopeds
    ...
    for (d = 1; d < num_mopeds; d++) {
        ...
        if (moped[d].left2) {
            moped[d].left3 = moped[d].left3 + 1;
            if (moped[d].right3 > 0) moped[d].right3 = moped[d].right3 - 1;
        }
        if (moped[d].right2) {
            moped[d].right3 = moped[d].right3 + 1;
            if (moped[d].left3 > 0) moped[d].left3 = moped[d].left3 - 1;
        }
        if (moped[d].left3 > 30) moped[d].left3 = 30;
        if (moped[d].right3 > 30) moped[d].right3 = 30;
        if (moped[d].up2) {
            moped[d].up3 = moped[d].up3 + 1;
            if (moped[d].up3 > 0) moped[d].up3 = moped[d].up3 - 1;
        }
        if (moped[d].down2) {
            moped[d].down3 = moped[d].down3 + 1;
            if (moped[d].down3 > 0) moped[d].down3 = moped[d].down3 - 1;
        }
...
{ // render workshop
    ...
    taka->BltFast(
        int(mousex), int(mousey), pictures[1],
        &loota,
        DDBLTFAST_SRCCOLORKEY);
    ...
    for (b = 0; b < 4; b++) {
        ...
        if ((ase[moped[0].ase[b]].prate_of_fire > 5) && (ase[moped[0].ase[b]].prate_of_fire) < 8) {
            ase[moped[0].ase[b]]._type            = 2;
            nop                                   = 2;
            ase[moped[0].ase[b]].bullets_per_shot = (int)(7 * (ase[moped[0].ase[b]].prate_of_fire - 3));
            ase[moped[0].ase[b]].damage           = ase[moped[0].ase[b]].damage / 20;
            ase[moped[0].ase[b]].dispersion       = ase[moped[0].ase[b]].dispersion * 4;
        }
...
void readkey_game(void)
{
    ...
    } else if (gamespeed < 0.9f + 0.1f * game_difficulty)
        gamespeed = gamespeed + 0.5f * 0.01f * elapsed;
...
{ // load game
    ...
    fgets(temprow, sizeof(temprow), fil);
    moped[0].x1 = (float)atof(temprow);
    fgets(temprow, sizeof(temprow), fil);
    moped[0].z1 = (float)atof(temprow);

    // rearwheel
    fgets(temprow, sizeof(temprow), fil);
    moped[0].x2 = (float)atof(temprow);
    fgets(temprow, sizeof(temprow), fil);
    moped[0].z2 = (float)atof(temprow);
    // direction
    fgets(temprow, sizeof(temprow), fil);
    moped[0].direction = (float)atof(temprow);
    // ammount of money
    fgets(temprow, sizeof(temprow), fil);
    moped[0].money = atoi(temprow);
    // weapon properties
    for (q = 0; q < 4; q++) {
        fgets(temprow, sizeof(temprow), fil);
        ase[moped[0].ase[q]].pdamage = atoi(temprow);
        fgets(temprow, sizeof(temprow), fil);
        ase[moped[0].ase[q]].pspeed = atoi(temprow);
        fgets(temprow, sizeof(temprow), fil);
        ase[moped[0].ase[q]].prate_of_fire = atoi(temprow);
        fgets(temprow, sizeof(temprow), fil);
    ...
    // energy
    fgets(temprow, sizeof(temprow), fil);
    moped[0].energy = (float)atof(temprow);
    // what level missions are given
    fgets(temprow, sizeof(temprow), fil);
    moped[0].level = atoi(temprow);
    // what mission from this level has been given
    fgets(temprow, sizeof(temprow), fil);
    moped[0].missionumero = atoi(temprow);
    // mission
    ...
    fgets(temprow, sizeof(temprow), fil);
    moped[0].mission.kohteita = atoi(temprow);
    // targets coordinates
    for (q = 0; q < moped[0].mission.kohteita; q++) {
        fgets(temprow, sizeof(temprow), fil);
        kx = (float)atof(temprow);
        fgets(temprow, sizeof(temprow), fil);
        kz = (float)atof(temprow);
    ...
    // is it accomplished
    fgets(temprow, sizeof(temprow), fil);
    moped[0].mission.status = atoi(temprow);
    // time
    fgets(temprow, sizeof(temprow), fil);
    moped[0].mission.timer = (float)atof(temprow);
    ...
    fgets(temprow, sizeof(temprow), fil);
    moped[0].mission.asiakas[0].picture = atoi(temprow);
    // energy
    fgets(temprow, sizeof(temprow), fil);
    moped[0].mission.asiakas[0].energy = (float)atof(temprow);
...
{ // new game
    ...
    for (b = 0; b < 12; b++) {
        ...
        if ((ase[b].prate_of_fire > 5) && (ase[b].prate_of_fire) < 8) {
            ase[b]._type            = 2;
            nop                     = 2;
            ase[b].bullets_per_shot = (int)(7 * (ase[b].prate_of_fire - 3));
            ase[b].damage           = ase[b].damage / 20;
            ase[b].dispersion       = ase[b].dispersion * 4;
        }
