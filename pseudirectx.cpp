...
HRESULT DIRECTDRAWSURFACE7::Blt(LPRECT dstRect, DIRECTDRAWSURFACE7*& src, LPRECT srcRect, DWORD dwFlags, void* blt_fx)
{
    ...
    glBegin(GL_QUADS);
    glTexCoord2f(srcRect->left / src->w, 1.0 - srcRect->top / src->h);
    glVertex2f(dstRect->left, dstRect->top);
    glTexCoord2f(srcRect->right / src->w, 1.0 - srcRect->top / src->h);
    glVertex2f(dstRect->right, dstRect->top);
    glTexCoord2f(srcRect->right / src->w, 1.0 - srcRect->bottom / src->h);
    glVertex2f(dstRect->right, dstRect->bottom);
    glTexCoord2f(srcRect->left / src->w, 1.0 - srcRect->bottom / src->h);
...
void DIRECT3DDEVICE7::SetRenderState(int key, DWORD value)
{
    ...
    case D3DRENDERSTATE_ZBIAS:
        glPixelTransferf(GL_DEPTH_BIAS, value);
        break;
    case D3DRENDERSTATE_ZWRITEENABLE:
        glDepthMask(value == TRUE ? GL_TRUE : GL_FALSE);
        break;
    default:
        throw MyEx("unknown key for SetRenderState");
    }
}
